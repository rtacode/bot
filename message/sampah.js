const { getBuffer, reSize, getGroupAdmins } = require("../Maslent/lib/myfunc")
const { color } = require("../Maslent/lib/color");
...
const textWelcome = `${global.textWelcome}`

const textLeave = `${global.textleft}`

const textPormote = `< PROMOTE >\n\nNama : ${pushname}\nNumber : @${memNumber}`

const textDemote = `< DEMOTE >\n\nNama : ${pushname}\nNumber : @${memNumber}`
...
//Button welcome
let buttonWelcome = [
{ buttonId: `y`, buttonText: { displayText: `WELCOME` }, type: 'RESPONSE' },
]

//Button leave
let buttonLeave = [
{ buttonId: `y`, buttonText: { displayText: `BYE` }, type: 'RESPONSE' },
]

//Button promote
let buttonPromote = [
{ buttonId: `y`, buttonText: { displayText: `CONGRATULATIONS` }, type: 'RESPONSE' },
]

//Button demote
let buttonDemote = [
{ buttonId: `y`, buttonText: { displayText: `SABAR YAH` }, type: 'RESPONSE' },
]
...
if (Remove&& OneMem){
console.log(chalk.bold.rgb(255, 178, 102)('\x1b[1;31m~\x1b[1;37m> [\x1b[1;32mGROUP\x1b[1;37m]'), color(`${pushname} Left the Group`,`yellow`), color(`${groupName}`,`blue`))
} else  if (Add && OneMem){ 
console.log(chalk.bold.rgb(255, 178, 102)('\x1b[1;31m~\x1b[1;37m> [\x1b[1;32mGROUP\x1b[1;37m]'), color(`${pushname} Has Ben Join The Group`,`blue`), color(`${groupName}`,`yellow`))
}
